const STORAGE_KEY = 'khallak_raed_v4';
...
function freshState(){
  return {
    stage:'home',
    teams:[
      {name:'الصقور',score:0,aids:['double','pit','time'],usedAids:[]},
      {name:'الفرسان',score:0,aids:['double','pit','time'],usedAids:[]}
    ],
    timer:20,
    selected:[],
    pickerTurn:0,
...
function finishGame(){
  ...
  if(tie){$('winnerTitle').textContent='تعادل ناري!';$('winnerText').textContent='النقاط متساوية — استخدموا السؤال الحاسم لتحديد الرائد.';$('winnerIcon').textContent='⚔️';}
  else{const winner=a.score>b.score?a:b;$('winnerTitle').textContent=`${winner.name} هو الرائد!`;$('winnerText').textContent=`أنهى التحدي برصيد ${winner.score} نقطة. مباراة قوية 👑`;$('winnerIcon').textContent='🏆';confetti();beep('win');}
  saveGame();
}
