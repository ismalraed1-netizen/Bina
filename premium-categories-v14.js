(function(){
  ...
  function syncUnlocked(){
    ...
    PREMIUM_CATEGORIES.forEach(cat=>{
      if(owned.has(cat.id)&&!QUESTION_BANK.some(x=>x.id===cat.id))QUESTION_BANK.push(cloneCat(cat));
    });
  }

  function storeMarkup(){
    const p=readProfile();const owned=new Set(p.ownedCategories);
    const cards=PREMIUM_CATEGORIES.map(cat=>{
      const has=owned.has(cat.id);
      return `<article class="premium-cat-card ${has?'owned':''} ${cat.accent}">
        <div class="premium-cat-art"><span>${cat.icon}</span><em>${esc(cat.tag)}</em></div>
        <div class="premium-cat-info">
          <div class="premium-cat-title"><b>${esc(cat.name)}</b>${has?'<span class="owned-chip">مفتوحة ✓</span>':'<span class="premium-chip">PREMIUM</span>'}</div>
          <p>${esc(cat.desc)}</p>
          <div class="premium-cat-meta"><span>6 أسئلة</span><span>200 • 400 • 600</span></div>
        </div>
        <button class="premium-cat-buy" data-premium-cat="${cat.id}" ${has?'disabled':''}>${has?'مملوكة':`${cat.cost} 🪙 شراء`}</button>
      </article>`;
    }).join('');
    return `<section id="premiumCategoryStore" class="premium-category-store">
      <div class="premium-store-head">
        <div><span>إضافات اللعبة</span><h4>فئات المتجر</h4><p>اشترِ الفئة مرة واحدة وتظهر بعدها مع الفئات في كل مباراة.</p></div>
        <div class="premium-store-count"><strong>${owned.size}/5</strong><small>فئات مفتوحة</small></div>
      </div>
      <div class="premium-cat-grid">${cards}</div>
    </section>`;
  }

  function buyCategory(id){
    const cat=PREMIUM_CATEGORIES.find(x=>x.id===id);if(!cat)return;
    if(typeof state!=='undefined'&&state?.stage==='game'){
      if(typeof toast==='function')toast('اشترِ الفئات قبل بدء المباراة أو بعد انتهائها');
      return;
    }
    const p=readProfile();
    if(p.ownedCategories.includes(id))return;
    ...
    if(typeof toast==='function')toast(`🎁 تم فتح فئة ${cat.name}! ستظهر الآن مع الفئات`);
  ...
  function bindStoreButtons(){
    document.querySelectorAll('[data-premium-cat]').forEach(btn=>{btn.onclick=()=>buyCategory(btn.dataset.premiumCat);});
  }

  function enhanceStore(){
    const modal=document.getElementById('v12Modal');if(!modal||modal.classList.contains('hidden'))return;
  ...
  function decoratePicker(){
    ...
    document.querySelectorAll('.pick-card').forEach(card=>{
      ...
      const cat=PREMIUM_CATEGORIES.find(x=>x.name===name);
      if(!cat)return;
      ...
      if(!owned.has(cat.id))card.remove();
    });
  }

  function wrapPicker(){
    if(typeof window.renderCategoryPicker==='function'&&!window.renderCategoryPicker.__premium14){
      const old=window.renderCategoryPicker;
      const fn=function(){const r=old.apply(this,arguments);setTimeout(decoratePicker,0);return r;};
      fn.__premium14=true;window.renderCategoryPicker=fn;
    }
  }
  ...
  function init(){
    ...
    const obs=new MutationObserver(()=>{enhanceStore();wrapPicker();setVersion();});
    obs.observe(document.body,{childList:true,subtree:true});
    setTimeout(()=>{syncUnlocked();enhanceStore();decoratePicker();wrapPicker();setVersion();},500);
  }

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
  window.addEventListener('load',()=>{syncUnlocked();enhanceStore();decoratePicker();setVersion();});
