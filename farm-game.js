...
function plotProgress(plot){if(!plot.crop||!plot.plantedAt)return 0;const crop=CROPS[plot.crop];const target=crop.grow*(plot.watered?.66:1);return clamp((Date.now()-plot.plantedAt)/target,0,1)}
...
function moveFarmerTo(plotEl,action,after){if(farmerBusy)return;farmerBusy=true;const world=$('#farmWorld'),farmer=$('#farmer');const pr=plotEl.getBoundingClientRect(),wr=world.getBoundingClientRect();const x=pr.left-wr.left+pr.width/2-35;const y=pr.top-wr.top+pr.height-94;farmer.style.transform=`translate(${x}px,${y}px)`;setTimeout(()=>{if(action==='water'){farmer.classList.add('watering');setTimeout(()=>farmer.classList.remove('watering'),900)}after&&after();setTimeout(()=>farmerBusy=false,650)},650)}
function handlePlot(i,el){const plot=state.plots[i];const progress=plotProgress(plot);const tool=state.selectedTool;
